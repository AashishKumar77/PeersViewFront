...
export class SharedJobComponent {
  ...
  protected notJobMessage: string = 'No Jobs Posted.';
  ...
  public ngOnChanges (changes: SimpleChanges): void {
    if (this.jobs.length === 0 && changes.jobs.previousValue) {
      this.notJobMessage = 'No Jobs Posted.';
    }
  }
  ...
  protected onDeleteJob (jobId: number): void {
    ...
    dialogConfig.panelClass = 'job-delete-confirm-modal';
  ...
  protected onEditJob (jobid: number): Promise<boolean> {
    let jobId = CryptoUtilities.cipher(jobid);

    return this.router.navigate([`/compose-job/${jobId}`]);
  }
