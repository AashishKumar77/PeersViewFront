...
import {
  SharedPostReplyCommentModalComponent, SharedImagePreviewComponent
} from '../../modals';
import {
  CryptoUtilities
} from '../../utilities';
...
export class SharedPostReplyComponent  {
  ...
  @Output() protected onClose = new EventEmitter<void>();

  private user: UserModel = UserService.getUser();
  protected isUserCurrentlyCommenting = false;
  protected isToogleUploadComponentVisible = false;
  ...
  protected onAddPostReply (): any {
    if (!this.postReply.comment) {
      return MessageNotificationService.show({
        notification: {
          id: 'shared-post-textarea-message',
          message: 'Cannot Reply Post',
        ...
        this.postApiService.promiseCreatePostReply(this.post.id, this.postReply)
          .then(async (response: IResponse) => {
            ...
            if (response && response.data) {
              ...
              postReply.attachments = response.data.attachments;
  ...
  protected onUploadComplete (attachments): void {
    switch (this.route.name) {
      case 'home':
        this.postReply.attachments = attachments;
        break;
      case 'campus':
      case 'campusFreshersFeed':
        this.campusPostReply.attachments = attachments;
        break;
    }
