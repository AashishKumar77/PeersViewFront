...
export class AngularEditorService {
  ...
  public saveSelection (): void {
    if (that.doc && that.doc.getSelection) {
      const sel = that.doc.getSelection();
      if (sel.getRangeAt && sel.rangeCount) {
        that.savedSelection = sel.getRangeAt(0);
        that.selectedText = sel.toString();
      }
    } else if (that.doc && that.doc.getSelection && that.doc.createRange) {
      that.savedSelection = document.createRange();
    } else {
      that.savedSelection = null;
    }
  }
  ...
  public setDefaultParagraphSeparator (separator: string): void {
    this.doc.execCommand('defaultParagraphSeparator', false, separator);
  }

  public createCustomClass (customClass: CustomClass): void {
  ...
  public insertVideo (videoUrl: string): void {
  ...
  public getRangeSelectedNodes (range, includePartiallySelectedContainers): any {
  ...
  public replaceWithOwnChildren (el): void {
  ...
  public removeSelectedElements (tagNames): void {
