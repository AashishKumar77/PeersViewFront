...
export class AngularEditorComponent implements OnInit, ControlValueAccessor, AfterViewInit, OnDestroy {
  ...
  private focusInstance: any;
  private blurInstance: any;

  @Input() private id = '';
  @Input() private config: AngularEditorConfig = angularEditorConfig;
  @Input() private placeholder = '';
  @Input() private tabIndex: number | null;

  @Output() private html;

  @ViewChild('editor') private textArea: ElementRef;
  @ViewChild('editorWrapper') private editorWrapper: ElementRef;
  @ViewChild('editorToolbar') private editorToolbar: AngularEditorToolbarComponent;

  @Output() private viewMode = new EventEmitter<boolean>();

  /** emits `blur` event when focused out from the textarea */
    // tslint:disable-next-line:no-output-native no-output-rename
  @Output('blur') private blurEvent: EventEmitter<FocusEvent> = new EventEmitter<FocusEvent>();

  /** emits `focus` event when focused in to the textarea */
    // tslint:disable-next-line:no-output-rename no-output-native
  @Output('focus') private focusEvent: EventEmitter<FocusEvent> = new EventEmitter<FocusEvent>();

  @HostBinding('attr.tabindex') private tabindex = -1;
   ...
   */
  public togglePlaceholder (value: boolean): void {
    if (!value) {
   ...
   */
  public toggleEditorMode (bToSource: boolean): void {
    let oContent: any;
