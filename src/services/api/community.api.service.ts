...
export class CommunityApiService extends ApiService {
  ...
  /**Answer question */
  public promiseCreateAnswerToQuestionByAnonymous (answer: CommunityAnswerQuestionModel): Promise<CommunityAnswerQuestionModel> {
    return this.promisePostModelData(`${answer.questionId}/reply/anonymous`, answer)
      .then((response: IResponse) => {
        return CommunityFactory.createCommunityAnswerToQuestion(response.data);
      });
  }
