...
export class UserApiService extends ApiService {
  ...
  public promiseGetFollowers (userId: number): Promise<UserModel[]> {
    return this.promiseGetResponseData(`followers/${userId}`)
      .then((response: IResponse) => {
        return UserFactory.createMany(response.data);
      });
  }
  ...
  public promiseVerifyConnection (jotToken: string): Promise<IResponse> {
    return this.promisePostData(`verify-connection/${jotToken}`, {token: jotToken})
      .then((response: IResponse) => {
        return response;
      });
  }
  ...
  public promiseForgotPassword (data: any): Promise<IResponse> {
    return this.promisePostData('forgot-password', data)
      .then((response: IResponse) => {
        return response;
      });
  }

  public promiseResetPassword (jotToken: string, data: any): Promise<UserModel> {
    return this.promisePutData(`password-reset/${jotToken}`, data)
      .then((response: IResponse) => {
        return UserFactory.create(response.data);
      });
  }
  ...
  public promiseGetSelectedUsersByCompany (userId?: number): Promise<UserModel[]> {
    let params: HttpParams;
    return this.promiseGetResponseData(`${userId}/company`)
      .then((response: IResponse) => {
        return response.data;
      });
  }

  public promiseGetAllCompanyUsers (userId?: number): Promise<UserModel[]> {
    let params: HttpParams;
    return this.promiseGetAllResponseData(`${userId}/company`)
      .then((response: IResponse) => {
        return response.data;
      });
  }

  public promisePostCompanyUser (userId, userCompany: any): Promise<IResponse> {
   return this.promisePostData(`${userId}/company`, {userCompanyId: userCompany.id})
    .then((responseData: IResponse) => {
      return responseData;
    });
  }

  public promiseDeleteCompanyUser (id: number): Promise<IResponse> {
    return this.promiseRemoveData(`${id}/company`)
    .then((responseData: IResponse) => {
      return responseData;
    });
  }
